...
import type { TeamMessageAttachmentInput, TeamReportPayload, TeamReportRange } from './teamTypes';
...
function normalizeExplicitApiUrl(rawUrl: string) {
  const trimmed = rawUrl.trim();
  if (!trimmed) {
    return trimmed;
  }

  return hasUrlProtocol(trimmed) ? trimmed : `${inferProtocol(trimmed)}${trimmed}`;
}
...
function resolveApiBaseUrl() {
  ...
  if (explicitUrl) {
    return withApiSuffix(resolveLocalhostForNative(normalizeExplicitApiUrl(explicitUrl)));
  }
...
export function createTeamMessage(payload: {
  body: string;
