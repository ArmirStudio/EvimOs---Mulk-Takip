...
export default function TeamMeetingsPanel({ meetings, loading, error, isManager, onRefresh }: Props) {
  ...
  const handleCreate = async () => {
    ...
    if (!scheduledDate.trim() || !scheduledTime.trim()) {
      Alert.alert('Tarih/saat gerekli', 'Lütfen toplantı tarih ve saatini girin. (GG.AA.YYYY ve SS:DD)');
      return;
    }
    ...
    if (!day || !month || !year || !hour || !minute) {
      Alert.alert('Geçersiz format', 'Tarih: GG.AA.YYYY, Saat: SS:DD formatında girin.');
      return;
    }
          ...
          <Pressable style={styles.sheet}>
            <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
              <ScrollView showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
                ...
                <Text style={styles.fieldLabel}>Saat * (SS:DD)</Text>
                <TextInput
                  style={styles.input}
                  value={scheduledTime}
