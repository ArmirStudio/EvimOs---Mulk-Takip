        ...
        >
          {contactsLoading ? (
            ...
            <View style={s.emptyEmp}>
              ...
              <Text style={s.emptyEmpSub}>Sağ üstten yeni usta kaydı ekleyin</Text>
            </View>
          ) : (
        ...
        <Modal visible={showAddTypeModal} transparent animationType="slide" onRequestClose={() => setShowAddTypeModal(false)}>
          <TouchableOpacity style={s.modalOverlay} activeOpacity={1} onPress={() => setShowAddTypeModal(false)}>
            <View style={s.addTypeSheet}>
              ...
              <View style={s.hiddenDivider} />
              <TouchableOpacity
                style={s.hiddenOption}
...
const useStyles = createThemedStyles((theme) => StyleSheet.create({
  ...
  header:          { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 20, paddingTop: 16, paddingBottom: 12, gap: 12 },
  headerTitle:     { flex: 1, fontSize: 24, fontWeight: '700', color: theme.colors.textPrimary, textAlign: 'center' },
  headerBackBtn:   { width: 36, height: 36, borderRadius: 18, backgroundColor: theme.colors.surface2, alignItems: 'center', justifyContent: 'center', borderWidth: 1, borderColor: theme.colors.border },
  headerBackSpacer:{ width: 36, height: 36 },
