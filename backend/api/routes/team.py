...
TEAM_MESSAGE_FILE_BUCKET = "team-message-files"
MAX_TEAM_MESSAGE_ATTACHMENTS = 5
MAX_TEAM_MESSAGE_ATTACHMENT_BYTES = 10 * 1024 * 1024
TEAM_MESSAGE_ATTACHMENT_KINDS = {"image", "document", "file"}
BANNED_TEAM_MESSAGE_MIME_PREFIXES = ("audio/", "video/")
...
def _validate_task_payload(
    ...
    *,
    ...
    assignee_provided = payload.assignee_id is not None or "assignee_id" in payload_fields
    assignee_id = (
        payload.assignee_id
        if assignee_provided
        else existing_task.get("assignee_id") if existing_task else None
    )
...
def _fetch_message_attachments(message_ids: list[str]) -> dict[str, list[dict]]:
    normalized_ids = [item for item in set(message_ids) if item]
    if not normalized_ids:
        return {}

    rows = (
        supabase.table("team_message_attachments")
        .select("*")
        .in_("message_id", normalized_ids)
        .order("created_at", desc=False)
        .execute()
        .data
        or []
...
def _validate_reply_message(reply_to_id: Optional[str], office_owner_id: str) -> None:
    if not reply_to_id:
        return

    result = (
        supabase.table("team_messages")
        .select("id, office_owner_id")
        .eq("id", reply_to_id)
        .maybe_single()
        .execute()
    )
    if not result.data or result.data.get("office_owner_id") != office_owner_id:
        raise HTTPException(status_code=400, detail="Yanitlanan mesaj ofisinize ait degil")


def _validate_message_attachments(attachments: list, office_owner_id: str, user_id: str) -> list[dict]:
    if len(attachments) > MAX_TEAM_MESSAGE_ATTACHMENTS:
        raise HTTPException(status_code=400, detail="Bir mesaja en fazla 5 ek eklenebilir")

    validated = []
    expected_prefix = f"{office_owner_id}/{user_id}/"

    for attachment in attachments:
        bucket = (attachment.bucket or "").strip()
        storage_path = (attachment.storage_path or "").replace("\\", "/").strip().lstrip("/")
        file_name = (attachment.file_name or "").strip()
        mime_type = (attachment.mime_type or "application/octet-stream").strip().lower()
        size_bytes = attachment.size_bytes
        kind = attachment.kind

        if bucket != TEAM_MESSAGE_FILE_BUCKET:
            raise HTTPException(status_code=400, detail="Desteklenmeyen mesaj eki bucket")
        if not storage_path or ".." in storage_path.split("/"):
            raise HTTPException(status_code=400, detail="Gecersiz ek dosya yolu")
        if not storage_path.startswith(expected_prefix):
            raise HTTPException(status_code=403, detail="Ek dosya yolu ofis kullanicisiyle eslesmiyor")
        if not file_name:
            raise HTTPException(status_code=400, detail="Ek dosya adi zorunludur")
        if len(file_name) > 180:
            raise HTTPException(status_code=400, detail="Ek dosya adi cok uzun")
        if any(mime_type.startswith(prefix) for prefix in BANNED_TEAM_MESSAGE_MIME_PREFIXES):
            raise HTTPException(status_code=400, detail="Ses ve video dosyalari desteklenmiyor")
        if size_bytes is not None and (size_bytes <= 0 or size_bytes > MAX_TEAM_MESSAGE_ATTACHMENT_BYTES):
            raise HTTPException(status_code=400, detail="Ekler en fazla 10 MB olabilir")
        if kind not in TEAM_MESSAGE_ATTACHMENT_KINDS:
            raise HTTPException(status_code=400, detail="Gecersiz ek tipi")
...
def _serialize_message(
    message: dict,
    user_map: dict[str, dict],
    reply_map: dict[str, dict] | None = None,
    attachments_map: dict[str, list[dict]] | None = None,
) -> dict:
    ...
    if reply_to_id and reply_map:
        ...
        if ref:
            ...
            ref_body = (ref.get("body") or "").strip()
            if not ref_body and ref.get("attachments"):
                ref_body = "Ek gonderildi"
            reply_to = {
                "id": reply_to_id,
                "body": ref_body[:100],
    ...
    attachments = (
        attachments_map.get(message.get("id"), [])
...
def list_team_messages(current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        attachments_map = _fetch_message_attachments([message.get("id") for message in messages])
        for message in messages:
            message["attachments"] = attachments_map.get(message.get("id"), [])
        reply_map = {m["id"]: m for m in messages}
        return {"messages": [_serialize_message(message, user_map, reply_map, attachments_map) for message in messages]}
    except Exception:
...
def create_team_message(request: CreateTeamMessageRequest, current_user: dict = Depends(get_current_user)):
    ...
    attachments = _validate_message_attachments(request.attachments or [], office_owner_id, current_user["id"])

    if not body and not attachments:
        raise HTTPException(status_code=400, detail="Mesaj veya ek zorunludur")
    if len(body) > 2000:
        raise HTTPException(status_code=400, detail="Mesaj en fazla 2000 karakter olabilir")
    _validate_reply_message(request.reply_to_id, office_owner_id)
    ...
    if attachments:
        ...
        try:
            ...
            try:
                ...
                logger.exception("Team message cleanup failed for %s", created_message.get("id"))
            raise HTTPException(status_code=500, detail="Mesaj ekleri kaydedilemedi") from exc

    created_message["attachments"] = created_attachments
    ...
    notify_users(
        ...
        body[:120] if body else "Ek gonderildi",
        created_message["id"],
    )
...
def create_team_task(request: CreateTeamTaskRequest, current_user: dict = Depends(get_current_user)):
    ...
    assignee = _validate_assignee(created_task["assignee_id"], office_owner_id) if created_task.get("assignee_id") else None
    if assignee:
        notify_user(
...
def transition_team_task(task_id: str, request: TeamTaskTransitionRequest, current_user: dict = Depends(get_current_user)):
    ...
    updated_task = result.data
    assignee = _validate_assignee(updated_task["assignee_id"], office_owner_id) if updated_task.get("assignee_id") else None

    if request.action == "complete" and assignee:
