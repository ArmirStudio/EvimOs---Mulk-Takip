...
`legal-acceptance` cagrisi `terms_accepted_at = now()` ve `first_login = false` yazar. Bu akis admin olmayan aktif kullanicilar icin de normal backend oturumu ile calisir.

## Admin Endpointleri
Admin mutation akislari backend `/api/admin/*` altindadir. Admin-web reklam kampanyasi yonetiminin kanonik arayuzudur; mobil admin platform operasyonlari ve gecici dev araclari icindir.
